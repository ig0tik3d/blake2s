...
static enum sha256_algos opt_algo = ALGO_BLAKE2;
...
static void *miner_thread(void *userdata)
{
	...
	while (1) {
		...
		if (max64 <= 0)
			...
			if (opt_algo == ALGO_BLAKE2) max64 = 0x7ffffLL; 
			}
		...
		case ALGO_SHA256D:
			...
			break;
		case ALGO_BLAKE2:
			rc = scanhash_blake2(thr_id, work.data, work.target,
			                      max_nonce, &hashes_done);
			break;		
		default:
			/* should never happen */
...
int main(int argc, char *argv[])
{
	...
	printf("\xBA  blake2s cpu-miner (v1.1)                        \xBA\n");
	printf("\xBA  authors: ig0tik3d                               \xBA\n");
	printf("\xBA                                                  \xBA\n");
